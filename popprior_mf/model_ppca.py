...
class PPCA(ObservedDistribution):
    ...
    def __init__(
        ...
        # raise ValueError('This should not be constant - Make it a positive learnable parameter')
        ...
        assert (
            var_family == "normal"
        ), f"var_family must be normal, but got {var_family}"
    ...
    def get_data_log_likelihood(self, samples, counts, holdout_mask):
        ...
        if use_old:
            ...
            data_log_likelihood = torch.sum(torch.mul(data_log_likelihood, 1 - holdout_mask.to_dense()), dim=tuple(range(1, data_log_likelihood.dim())),)
    ...
    #         if log_ll_old is not None:
    #             assert torch.sum(torch.abs(log_ll_old - log_ll)) > 0, "log_ll_old is the same as log_ll"
    #         log_ll_old = log_ll
    ...
    @torch.no_grad()
    def compute_heldout_loglikelihood_none_zero(self, x_vad, n_monte_carlo=100, write_llhood=True):
    ...
    def compute_heldout_loglikelihood(
        ...
        if ignore_zeros:
            ...
            return self.compute_heldout_loglikelihood_none_zero(
                x_vad, n_monte_carlo=n_monte_carlo, write_llhood=write_llhood
            )
