...
class PMFNatural(ObservedDistribution):
    ...
    def __init__(
        ...
        super(PMFNatural, self).__init__()
        self.device = device
    ...
    def _setup_row_vars(
        ...
        self.qu_distribution = VariationalFamilyNatural(
            device=self.device,
            family=family,
    ...
    def _setup_column_vars(
        ...
        self.qv_distribution = VariationalFamilyNatural(
            device=self.device,
            family=family,
    ...
    @staticmethod
    def _match_lognormal_mean(mean, fixed_eta2, nugget=1e-8):
        """
        Finds the `location` (mu) of the lognormal distribution  with the given mean, for fixed variances.
        Expected value of the lognormal distribution: exp(mu + sigma^2/2), which with natural parameterization is:
        E[X] = exp(-.5 * eta_1/eta_2 - .25/eta_2)
        Then its eta_1 parameter for a desired mean is:
            eta_1 = -2 * eta_2 * ln(mean) - .5        
        """
        assert torch.all(fixed_eta2 <= 0.0), f"fixed_eta2 must be negative, but is {fixed_eta2}."
        # if mean is zero, then eta_1 is -0.5
        def safe_log(x):
        ...
        #return -2 * fixed_eta2 * torch.log(mean) - 0.5
        return -2 * fixed_eta2 * safe_log(mean) - 0.5
    ...
    @property
    def row_mean(self):
        return self.row_distribution._compute_mean()

    @property
    def column_mean(self):
        return self.column_distribution._compute_mean()

    def init_column_vars(self, w_init):
        # match the location to lognormal
        # convert log_minus_eta2 to eta2
        # TODO: make numerically stable

        # assume eta2 is all -1 --> ln_minus_eta2 is all 0
        self.column_distribution.ln_minus_eta2.data = torch.zeros_like(self.column_distribution.ln_minus_eta2)
        eta2 = -VariationalFamilyNatural.make_positive(self.column_distribution.ln_minus_eta2, exponentiate=False)
        #init_eta1 = PMFNatural._match_lognormal_mean(torch.from_numpy(w_init).to(self.device), eta2, nugget=0)
        init_eta1 = PMFNatural._match_lognormal_mean(torch.from_numpy(w_init), eta2.cpu(), nugget=0)
        self.column_distribution.eta1.data = torch.DoubleTensor(init_eta1).to(self.device)
        # TEST: compare the empirical mean with the w_init
        #breakpoint()
        #assert np.allclose(w_init, self.column_mean, atol=1e-2), f"empirical mean does not match w_init {w_init - self.column_mean}"
        assert isinstance(self.column_distribution.eta1, torch.nn.parameter.Parameter)

    def init_row_vars(self, z_init):
        self.row_distribution.ln_minus_eta2.data = torch.zeros_like(self.row_distribution.ln_minus_eta2)
        eta2 = -VariationalFamilyNatural.make_positive(self.row_distribution.ln_minus_eta2)
        #init_eta1 = PMFNatural._match_lognormal_mean(torch.from_numpy(z_init).to(self.device), eta2, nugget=0)
        init_eta1 = PMFNatural._match_lognormal_mean(torch.from_numpy(z_init), eta2.cpu(), nugget=0)
        self.row_distribution.eta1.data = torch.DoubleTensor(init_eta1).to(self.device)
        assert isinstance(self.row_distribution.eta1, torch.nn.parameter.Parameter)
    ...
    def forward(self, datapoints_indices, data, holdout_mask, step, use_sparse=True):
        ...
        # Remove the term for W
        # for p in self.column_distribution.parameters():
    ...
    def compute_heldout_loglikelihood(
        ...
        if use_mean:
            norm_constant = 0.0
            v = self.qv_distribution._compute_mean(convert_numpy=False)
            u = self.qu_distribution._compute_mean(convert_numpy=False)
            log_ll = self.compute_llhood_sparse(x_vad, missing_indexes=b_hat, v=v, u=u)
